...
const statesList = [
  ...
  const fetchAppointments = async () => {
    try {
      const response = await axios.get(`http://findopendentist.onrender.com/appointments?officeId=${user.id}`);
  ...
  const updateOfficeInfo = async () => {
    try {
      const response = await axios.post("http://findopendentist.onrender.com/update-office-info", updatedInfo);
  ...
  const toggleSlot = async (slot) => {
    ...
    try {
      const response = await axios.post("http://findopendentist.onrender.com/update-availability", {
        email: user.email,
